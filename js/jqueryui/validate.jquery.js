...
jQuery.validator.addMethod("alphanumericCardNames", function (value, element) {
...
}, "Por favor ingresa tus datos sin caracteres especiales, Ã± o tildes");

jQuery.validator.addMethod("noSpecialsAddress", function (value, element) {
    return this.optional(element) || /^\w+$/i.test(value.replace(/\ /g, ""));
}, "Por favor ingresa tus datos sin caracteres especiales");

jQuery.validator.addMethod("noSpecialsEmail", function (value, element) {
    return this.optional(element) || /^\w+$/i.test(value.replace(/\ |@|\.|\-/g, ""));
}, "Por favor ingresa tus datos sin caracteres especiales");

jQuery.validator.addMethod("documentFormat", function (value, element, param) {
    switch (jQuery(param).val()) {
        case "CC":
...
}, "El nÃºmero de la tarjeta no corresponde a la franquicia");
...
jQuery.validator.addMethod("validCardNumber", function (value, element) {
    ...
    return ((sum % 10) == 0);
}, "NÃºmero de tarjeta de crÃ©dito no es vÃ¡lida");

jQuery.validator.addMethod("initialCreditCardNumber", function (value, element, param) {
    switch (jQuery(param).val()) {
        case "AM":
...
jQuery.validator.addMethod("ajaxAutocomplete", function (value, element) {
    var id = jQuery(element).attr('id').replace('Validate', '');
    if ((jQuery("#" + id).val() == '') || (value.indexOf("(" + jQuery("#" + id).val() + ")") < 0)) {
        return false;
    } else {
        return true;
    }
}, "Debe seleccionar una opciÃ³n de la lista");

jQuery.validator.addMethod("expiration", function (value, element) {
    var prefix = jQuery(element).attr('name').split('_');
    var current = new Date();
    if ((jQuery('[name^="' + prefix[0] + '_year]"]').val() <= current.getFullYear())
            && (parseInt(value) <= current.getMonth())) {
        return false;
    } else {
        return true;
    }
}, "La fecha de expiraciÃ³n debe ser mayor que la fecha actual");

jQuery.validator.addMethod("securityCode", function (value, element, param) {
    switch (jQuery(param).val()) {
        case "AM":
...
}, "Longitud del cÃ³digo de seguridad incorrecto");

jQuery.validator.addMethod("notRepeated", function (value, element) {
    var prefix = jQuery(element).attr('id').split('doc_num');
    var compares = jQuery('[id^="' + prefix[0] + 'doc_num"]');
    for (var i = 0; i < compares.length; i++) {
        if ((jQuery(compares[i]).val() == value) && (jQuery(compares[i]).attr('id') != prefix[0] + "doc_num" + prefix[1])) {
            return false;
        }
    }
    return true;
...
jQuery.validator.addMethod("gdsMaxLength", function (value, element, param) {
    var totalLength = 1 + value.length + jQuery(param[0]).val().length;
    if (totalLength > param[1]) {
        jQuery(param[0]).addClass('error');
        return false;
    } else {
        if (jQuery(element).hasClass('error')) {
            jQuery(element).removeClass('error');
            jQuery(param[0]).removeClass('error');
            jQuery(param[0]).valid();
        }
        return true;
    }
}, jQuery.validator.format("Ha superado el mÃ¡ximo de {1} caractÃ©res entre nombre y apellido del pasajero."));

var _docRestrictions = function (event, doc_type) {
    var key = event.which || event.keyCode;
    var controlKeys = [8, 9, 13];
    if (jQuery.browser.mozilla)
        controlKeys = controlKeys.concat([37, 38, 39, 40]);
    if (controlKeys.join(",").match(new RegExp(key))) {
        return;
    }
    switch (jQuery(doc_type).val()) {
        case "TI":
        case "LI":
        case "CC":
        case "NIT":
...
var _nameRestrictions = function (event) {
    var key = event.which || event.keyCode;
    var controlKeys = [8, 9, 13];
    if (jQuery.browser.mozilla)
        controlKeys = controlKeys.concat([37, 38, 39, 40]);
...
var _noSpecialRestrictions = function (event) {
    var key = event.which || event.keyCode;
    var controlKeys = [8, 9, 13];
    if (jQuery.browser.mozilla)
        controlKeys = controlKeys.concat([37, 38, 39, 40]);
...
jQuery.validator.addMethod('mypassword', function (value, element) {
 ...
 'La contraseña debe contener al menos un numérico y un carácter alfabético.');
...
jQuery.extend(jQuery.validator.messages, {
    ...
    creditcard: "Por favor, escribe un nÃºmero de tarjeta valido.",
